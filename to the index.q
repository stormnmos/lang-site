...
DDEESSCCRRIIPPTTIIOONN
       ...
       your globs before the shell) will be silently ignored. The _g_i_t _a_d_d command can be used to
...
CCOONNFFIIGGUURRAATTIIOONN
       ...
       in the exclude file are used in addition to those in info/exclude. See ggiittiiggnnoorree(5).
...
IINNTTEERRAACCTTIIVVEE MMOODDEE
       When the command enters the interactive mode, it shows the output of the _s_t_a_t_u_s subcommand,
       ...
       In general, when the prompt ends with a single _>, you can pick only one of the choices given
       ...
       status
           ...
           (if the working tree version were also different, _b_i_n_a_r_y would have been shown in place
           of _n_o_t_h_i_n_g). The other file, git-add--interactive.perl, has 403 lines added and 35 lines
       ...
       update
           ...
           with double _>_>, you can make more than one selection, concatenated with whitespace or
           ...
           choose 7,8,9 from the list. You can say _* to choose everything.
       ...
       revert
           This has a very similar UI to _u_p_d_a_t_e, and the staged information for selected paths are
           reverted to that of the HEAD version. Reverting new paths makes them untracked.

       add untracked
           This has a very similar UI to _u_p_d_a_t_e and _r_e_v_e_r_t, and lets you add untracked paths to the
           index.

       patch
           This lets you choose one path out of a _s_t_a_t_u_s like selection. After choosing the path,
...
EEDDIITTIINNGG PPAATTCCHHEESS
       ...
       list below describes some common things you may see in a patch, and which editing operations
       make sense on them.
       ...
       "undo" the change in the index. For example, introducing a new line into the index that is
       in neither the HEAD nor the working tree will stage the new line for commit, but the line
...
Git 2.1.4                                    12/11/2015                                  GIT-ADD(1)
